...
class ModelTraining:
    ...
    def set_model_xgboost(self):
        self.models.append(('XGBoost', xgb.XGBRegressor(early_stopping_rounds=50)))
    ...
    def evaluate_model(self, model_name, model, train_X, train_y, test_X, test_y, target_col):
        try:
            ...
            metrics = self.utils.evaluate_forecast(test_y, y_pred, False)
            names_folder = {
                'country': self.country,
                'store': self.store,
                'product': self.product,
